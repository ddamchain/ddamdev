...
const USER_AGENT = "ddamers-dist@0.0.0";
const TAG = "next";
...
(async function() {
    ...
    // Publish the GitHub release (currently alpha)
    const alpha = true;
    if (includeEthers) {
        ...
        // Publish the release
        const link = await createRelease(username, password, change.version, change.title, change.content, alpha);
