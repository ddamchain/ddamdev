...
async function generate() {
    ...
    let lastVersion = await npm.getPackageVersion("ddamers");
    ...
    let newSection = {
        title: `ddamers/v${ local.loadPackage("ddamers").version } (${utils.getDateTime(new Date())})`,
        underline: "-",
        body: [ ]
    }
    ...
    changes.forEach((change) => {
        ...
        let link = body.match(/(\((.*#.*)\))/)
        let commit = `[${ change.commit.substring(0, 7) }](https://github.com/ddamchain/ddamdev/commit/${ change.commit })`;
        if (link) {
            body = body.replace(/ *(\(.*#.*)\) */, "");
            link = link[2].replace(/#([0-9]+)/g, (all, issue) => {
                return `[#${ issue }](https://github.com/ddamchain/ddamdev/issues/${ issue })`;
            }) + "; " + commit;
        } else {
            link = commit;
        }
